    ...
    public class thief : UWClass
    {
        ...
        public static void FlagTheftToObjectOwner(uwObject illegallyusedobject, int objectowner)
        {
            ...
            if (StolenItemOwner != 0)
            {
                ...
                if ((objectowner >= 0) && (illegallyusedobject.owner <= 29))
                {
                    ...
                    //if this object is a container clear owner of it's contents
                    if (illegallyusedobject.OneF0Class == 8)
                    {
                        CallBacks.RunCodeOnObjectsInChain(methodToCall: ClearOwnership, obj: illegallyusedobject, objList: UWTileMap.current_tilemap.LevelObjects);
                    }
                }
            }
        }
        public static bool ClearOwnership(uwObject obj)
        {
            if (commonObjDat.canhaveowner(obj.item_id))
            {
                obj.owner = 0;
            }
            return false;
        ...
        public static bool AngerNPCByIllegalAction(int x, int y, uwObject critter, TileInfo tile, int srcIndex)
        {
            ...
            if (critterObjectDat.generaltype(critter.item_id) == (StolenItemOwner & 0x1F))
            {
                ...
                if (dist <= critterObjectDat.sightdetectionrange(critter.item_id) * critterObjectDat.sightdetectionrange(critter.item_id))
                {
