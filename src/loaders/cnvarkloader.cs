...
namespace Underworld
{
    public class cnvArkLoader : Loader
    {
            ...
            if (Loader.ReadStreamFile(cnv_ark_path, out byte[] cnv_ark))
            {
                ...
                for (int i = 0; i < NoOfConversations; i++)
                {
                    ...
                    if (add_ptr != 0)
                    {
                        ...
                        */
                        ConversationVM.conversations[i].CodeSize = (int)Loader.getAt(cnv_ark, add_ptr + 0x4, 16);
                        ConversationVM.conversations[i].StringBlock = (int)Loader.getAt(cnv_ark, add_ptr + 0xA, 16);
                        ConversationVM.conversations[i].NoOfMemorySlots = (int)Loader.getAt(cnv_ark, add_ptr + 0xC, 16);
                        ConversationVM.conversations[i].NoOfImportedGlobals = (int)Loader.getAt(cnv_ark, add_ptr + 0xE, 16);
                        ...
                        for (int f = 0; f < ConversationVM.conversations[i].NoOfImportedGlobals; f++)
                        {
                            ...
                            int len = (int)Loader.getAt(cnv_ark, funcptr, 16);
                            for (int j = 0; j < len; j++)
                            {
                                ConversationVM.conversations[i].functions[f].functionName += (char)Loader.getAt(cnv_ark, funcptr + 2 + j, 8);
                            }
                            ConversationVM.conversations[i].functions[f].ID_or_Address = (int)Loader.getAt(cnv_ark, funcptr + len + 2, 16);
                            ConversationVM.conversations[i].functions[f].import_type = (int)Loader.getAt(cnv_ark, funcptr + len + 6, 16);
                            ConversationVM.conversations[i].functions[f].return_type = (int)Loader.getAt(cnv_ark, funcptr + len + 8, 16);
                            funcptr += len + 10;
                        }
                        ...
                        {
                            ConversationVM.conversations[i].instuctions[counter++] = (short)Loader.getAt(cnv_ark, funcptr + c, 16);
                        }
                    }
                }
                Loaded = true;
        ...
        public static void LoadCnvArkUW2(string cnv_ark_path)
        {
            ...
            int NoOfConversations = (int)Loader.getAt(tmp_ark, 0, 32);

            ConversationVM.conversations = new ConversationVM.conversation[NoOfConversations];

            for (int i = 0; i < NoOfConversations; i++)
            {
                int compressionFlag = (int)Loader.getAt(tmp_ark, address_pointer + (NoOfConversations * 4), 32);
                int isCompressed = (compressionFlag >> 1) & 0x01;
                int add_ptr = (int)Loader.getAt(tmp_ark, address_pointer, 32);
                if (add_ptr != 0)
                {
                    if (isCompressed == 1)
                    {
